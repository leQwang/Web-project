const shipperRegistartionForm = document.querySelector('#registerShipper-form');
const shipperSubmitBtn = document.querySelector('#registerShipper-btn');

shipperSubmitBtn.addEventListener('click', (event) => {
    //Wait with submitting to server
    event.preventDefault(); 

    let errorMessage = validateShipperForm();

    if(errorMessage === ''){
        shipperRegistartionForm.submit();
    }
    else{
        alert(errorMessage);
    }
